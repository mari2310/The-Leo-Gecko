...
if ($idgecko > 0) {
    ...
    $query_insert = $mysqli->prepare("INSERT INTO inseto (inseto , idgecko) VALUES (?, ?)");
    $query_insert->bind_param('si', $inseto, $idgecko);

    if ($query_insert->execute()) {
        header("Location: confirmacaoinseto.php");
        exit;
    } else {
...
<html>
	...
	<body>
		...
		<div class="header" class="login-section">
			...
			Cadastro de Inseto para o Gecko <?= $nomeGecko; ?>
			<br>
			<br>
		</div>
		<div class="register-section">
			<form action="cadastroinseto.php" method="post">
				<input type="hidden" name="idgecko" value="<?= $idgecko ?>"> <!-- Campo oculto para o ID do Gecko -->
				<label for="inseto">Inseto:</label><br>
				<textarea class="lacuna" name="inseto" rows="4" required></textarea><br>
				<input type="submit" class="btn-cadastro" value="Cadastrar">
				<a href="inseto.php?idgecko=<?= $idgecko ?>" class="cancel-button">Cancelar</a>
			</form>
		</div>
